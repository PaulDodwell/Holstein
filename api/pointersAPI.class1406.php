	   ...
	   //link management
	  	 ...
	  	 protected function request_link() {
			if ($this->method == 'POST') {
				...
				$q = "INSERT INTO link ( dev1, dev2, status ) VALUES ( :dev1, :dev2, :status )";
		 ...
		 protected function link_status() {
			...
			if ($this->method == 'POST') {
				...
				$q = "UPDATE `link` SET `status` = :status $stamp WHERE `link_id` = :link"; 
				$t = time();
		...
		protected function remove_link() {
			if ($this->method == 'POST') {
				...
				$q = "delete from `link` WHERE `dev1` = :dev1 && `dev2` = :dev2 && `status` > :status"; 
				
				return $this->postData($q, array( ':dev1'=>$rqid, ':dev2'=>$id, ':status'=>0 ));
		 ...
		 protected function get_links() {
			if ($this->method == 'GET') {
				...
				$q="select * FROM `link` WHERE `dev1` = `$rqid` AND `status` = $status order by dev2 ASC";
